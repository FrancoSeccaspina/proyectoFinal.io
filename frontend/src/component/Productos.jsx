...
function Productos() {
  ...
  const getProducts = async ()=> {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_DOMAIN_HOST}/api/productos`, { credentials: 'include' });
      const data = await response.json();
  ...
  const handleDelete = async (id) => {
    if (window.confirm("¿Estás seguro de que querés eliminar esta producto?")) {
      try {
        await axios.delete(`${process.env.REACT_APP_BACKEND_DOMAIN_HOST}/api/productos/${id}`, { withCredentials: true });
          ...
          <tbody>
            {productosFiltrados.map((prod) => (
              <tr key={prod.id}>
                ...
                <td>
                  <img
                    src={`${process.env.REACT_APP_BACKEND_DOMAIN_HOST}/images/${prod.imagen}`}
                    alt={prod.nombre}
                    width='150'
